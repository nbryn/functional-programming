...
let main argv =
    ...
    printfn "%A" (boolEval ((V "x" .+. V "y") .=. (V "y" .-. V "x"))  |> 
                  evalSM state)
    printfn "%A" (boolEval (IsDigit (CV (V "x")))  |> 
                evalSM state)
    printfn "%A" (boolEval (IsLetter (CV (V "x" .-. N 1)))  |> 
                evalSM state)
    printfn "%A" (boolEval (IsDigit (CV (V "x" .-. N 1)))  |> 
                evalSM (mkState [] [('0', 42)] []))
   ...
   (*  printfn "Testing arithEval2:"
    ...
    printfn "%A" (boolEval2 ((V "x" .+. V "y") .=. (V "y" .-. V "x"))  |> 
                  evalSM state)
    printfn "%A" (boolEval2 (IsDigit (CV (V "x")))  |> 
                  evalSM state)
    printfn "%A" (boolEval2 (IsLetter (CV (V "x" .-. N 1)))  |> 
                  evalSM state)
    printfn "%A" (boolEval2 (IsDigit (CV (V "x" .-. N 1)))  |> 
                  evalSM (mkState [] [('0', 42)] []))
    ...
    printfn "%A" (containsNumbers hello 5 50)
    printfn "%A" (containsNumbers (('0', 100)::hello) 5 50)
    printfn "%A" (containsNumbers (('0', 100)::hello) 5 50)
    printfn ""
